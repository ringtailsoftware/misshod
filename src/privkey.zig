...
fn decodeAsciiToBinary(keydata_ascii: []const u8, keydata_bin_buf: []u8) ![]u8 {
    ...
    if (b64_slice_opt) |b64_slice| {
        ...
        if (decoded_size > keydata_bin_buf.len) {
            return error.PrivKeyOutofSpace;
        }
        const sz = decoder.decode(keydata_bin_buf, b64_slice) catch return error.BadPrivKey;
        return keydata_bin_buf[0..sz];
    } else {
        return error.BadPrivKey;
    }
}

pub fn decodePrivKey(keydata_ascii: []const u8, passphrase_opt: ?[]const u8, privkey_blob: *[srv_hostkey_algo.SecretKey.encoded_length]u8) !void {
    ...
    if (!std.mem.eql(u8, AuthMagic, bin[0..AuthMagic.len])) {
        return error.BadPrivKey; // Not magical enough
    }
    ...
    if (n_keys != 1) {
        return error.BadPrivKey; // All known files have a single keypair
    }
    ...
    if (std.mem.eql(u8, ciphername, "aes256-ctr")) {
        if (std.mem.eql(u8, kdfname, "bcrypt")) {
            if (passphrase_opt) |passphrase| {
                ...
                return error.InvalidKeyDecrypt;
            }
        } else {
            return error.UnsupportedPrivKey;
        }
    } else {
        if (!std.mem.eql(u8, ciphername, "none")) {
            return error.UnsupportedPrivKey; // aes256-ctr/none are only options we support
        }
    }
...
test "decodepriv" {
    ...
    try std.testing.expectError(error.BadPrivKey, decodePrivKey(testkey_invalid_bad_preamble, null, &blob));
    try std.testing.expectError(error.BadPrivKey, decodePrivKey(testkey_invalid_missing_footer, null, &blob));
    try std.testing.expectError(error.BadPrivKey, decodePrivKey(testkey_invalid_bad_base64, null, &blob));
    try decodePrivKey(testkey_encrypted_valid_passworded, testkey_encrypted_valid_password, &blob);
    try std.testing.expectError(error.InvalidKeyDecrypt, decodePrivKey(testkey_encrypted_valid_passworded, "notpassword", &blob));
